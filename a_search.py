...
def manhattan_distance(i,k):
    goal_states = [[2,6],[5,6],[4,7],[7,7]]
    results = []
    for states in goal_states:
        results.append( abs(states[0]-i) + abs(states[1]-k) )
    return min(results)/10
...
def a_star_search():
    ...
    node.pathCost = manhattan_distance(2,1) + 0
    ...
    for i in range(100):
        ...
        for move in moves:
            ...
            child.pathCost =  calc_path_cost(move) + manhattan_distance(move[0],move[1]) + current_node.pathCost 
            child.state = move
            if child.state not in explored_ or frontier_:
                if goalTest(child.state) == True:
                    ...
                    print("Goal Path : ")
                    print(goal_path)
                    print("Total Path Cost : %lf " % total_path_cost)
